				  ...
				  unsigned long arg)
{
	...
	case PB2_SET_ORDER:
	{
		...
		if((unsigned char)chdata != GRAPH_IN_ORDER && (unsigned char)chdata != GRAPH_POST_ORDER &&
			       	(unsigned char)chdata != GRAPH_PRE_ORDER)
		       	return -EINVAL;
	...
	case PB2_GET_OBJ:
	{
		...
		if((unsigned char) objptr.objtype == DATA_TYPE_INT) 
			node = search_graph(entry_ptr->graph, &objptr.int_obj);
		else 
			node = search_graph(entry_ptr->graph, objptr.str);
...
static __init int init_module_assign(void)
{
	...
	pr_info("Creating Process : partb_2_16CS30023_16CS10018\n");
...
{
	pr_info("Removing Process : partb_2_16CS30023_16CS10018\n");
